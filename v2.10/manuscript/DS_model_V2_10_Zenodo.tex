...
\date{September 2026 --- admin@dipolarstrings.org --- V2.10 (Zenodo edition)}
...
\paragraph{Closure of the size channel.} The topological loop radius $R_{\Ncd}$ is not identified with any measurable charge radius: the electron loop is 386~fm against a point-like measured electron ($<10^{-3}$~fm). The proton loop radius depends on which closure is adopted: the resonance-radius law of Eq.~\eqref{eq:resonance_radius} gives $17.0$~fm through the reparametrized ratio $Z_p/Z_e \simeq 22.7$, whereas the purely geometric closure $2\pi R_{\Ncd} = \Ncd\,\ell_1$ gives $3.5\times10^{3}$~fm. The measured charge radius is $0.841$~fm, so the loop radius exceeds it by a factor $\approx 20$ under the resonance closure and $\approx 4\times10^{3}$ under the geometric one. The resonance-radius law itself does not follow from the phase condition \mbox{$\oint\beta\,ds = 2\pi p$}, which relates $R$, $\omega$ and $v$ without fixing any of them. Absent a loop-to-observable rule, the mass law $m \propto \Ncd^2/R$ is unverifiable through sizes; this verification channel is declared closed.
...
This working formalism distinguishes six levels of theoretical maturity, and records separately, under a seventh heading, what the present revision withdraws:
\begin{itemize}
...
\paragraph{V2.9.2 (September 2026).} This edition makes two further corrections, both reducing the reach of existing statements. The closure of the size channel now reports the proton loop radius under both available closures --- the resonance-radius law and the purely geometric one --- and records that the resonance-radius law does not follow from the phase condition \mbox{$\oint\beta\,ds = 2\pi p$}, which relates $R$, $\omega$ and $v$ without fixing any of them. The epistemological placement of Theorem~4 is narrowed: the telegrapher correspondence is Maxwell's equations in one dimension written in circuit variables, so what the theorem establishes is the uniqueness of the Johns node under C1--C5 given its stated inputs, not a derivation of electromagnetism.

\paragraph{V2.10 (September 2026).} Three corrections, none of which changes a result; all three remove an internal inconsistency in favour of the more conservative reading. A leftover sentence in Section~\ref{sec:lrd} still named the shadow-diameter deviation as the model's primary target, contradicting its withdrawal in the same revision and the revised falsification hierarchy of Section~\ref{sec:discussion}; it now names the permanent absence of PBH evaporation bursts, the first entry of that hierarchy. The epistemological-status list announced six levels of maturity while presenting seven headings, the seventh being the record of what the revision withdraws rather than a level of maturity; the announcement is corrected rather than the list. The open-problem entry on the weave chirality was titled \emph{resolved, with a residual question}, which reads as a closed item: what is dissolved is the chiral closure of Theorem~4, whereas the converse question --- by what mechanism a chirality that is mute in the electromagnetic sector could acquire physical relevance --- remains open, and the title now says so.
