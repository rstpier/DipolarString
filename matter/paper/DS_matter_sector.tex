...
statement imposes and what it breaks; the record is 109 steps (R1--R109) backed by 104 scripts.
...
electromagnetic electron is Dirac's. Read instead as the quantum of the medium's field, the
electron is consistent with all of this (the telegrapher pair is a Weyl pair, the half quantum is
the antiperiodic sector of a closed line, $G_E = 1$ by construction, $g = 2$ from the first-order
structure), but on the base's cubic weave every spin-$\tfrac12$ excitation propagates at exactly
$2/3$ of the weave's light speed, excluded at $10^{-11}$.
...
Steps R104--R107 then test the reading the author proposed, the electron as the quantum of a
universal field, and find it consistent but not realizable on the base's cubic weave
(Section~\ref{sec:fieldquantum}). The sector's claims are therefore hadronic and generational.
...
The whole chain is recomputed in one script (\texttt{hadron\_sector.py}, R109) from the inputs,
with its sensitivities: moving the exponent across the window the lepton masses allow
($6.2758$--$6.2926$) shifts every hadronic number by less than $2\,\%$; the strand count is the
input that weighs (8 or 10 in place of 9 would give 364 or 1479~MeV for the static part); the
ribbon width enters $m_n - m_p$ only through a logarithm that nearly cancels between $u$ and $d$
($-12$ to $+9\,\%$ for a factor of two); the ring radius, $0.56$--$0.67$~fm, is the one number
the base does not fix and it moves the nucleon mass from 938 to 910~MeV. Nothing of the electron
enters but $m_e$ through $\lb$: the sector is independent of the exclusions of
Sections~\ref{sec:electron}--\ref{sec:fieldquantum}, and under the field reading its anchor
$2\pi\lb/3$ is a posed length. The consolidated chapter, with inputs, rules, chain, statuses and
what would kill the sector, is \texttt{matter/redraw/HADRONS.md}.
...
%------------------------------------------------------------------------------
\subsection{The electron as the quantum of the medium's field}\label{sec:fieldquantum}
%------------------------------------------------------------------------------

The author's proposal after R103: the electron has no locality when it is not observed; it is the
quantum of a universal field, the medium itself, spread like its mode and taken whole when it
interacts. The manuscript already says so (``the electron is the quantum of the line, not its
soliton''). Four steps test what this reading buys and what it costs
(\texttt{field\_quantum.py}, \texttt{weave\_doublet.py}, \texttt{node\_transport.py},
\texttt{polarized\_spinor\_node.py}).

\begin{enumerate}[nosep]
\item \textbf{What it buys (R104).} The telegrapher equations of a matched line are a first-order
two-component field: the eigenmodes of the symbol are $V \pm Z_0 I$ at $\pm c$ without dispersion,
$i\partial_t\psi = -ic\,\sigma_z\partial_x\psi$, the 1+1 Weyl pair. On a closed line of length
$L$ the spectrum is $\hbar c|k|$ with $k = 2\pi n/L$ (periodic sector) or $2\pi(n+\tfrac12)/L$
(antiperiodic): the mother's whole quantum $2\pi\hbar c/L$ (R87) is the periodic ground state, the
daughter's half quantum $\pi\hbar c/L$ (R47, R86) the antiperiodic one, and the $-1$ under a full
turn is the field's boundary condition. The antiperiodic ground mode carries $L_z = \hbar/2$ at
any radius; with the whole rest energy in the mode, $R = \lb/2$, the turn frequency is
$2mc^2/\hbar$ and the phase frequency $mc^2/\hbar$: the three numbers of Dirac's Zitterbewegung,
with no static half. The charge vertex of a field quantum is $|\langle k+q|\hat\rho(q)|k\rangle| = e$
for every $q$ whatever the extent of the mode: $F = 1$ by construction, against $j_0(qR)$ for a
classical distribution. For a two-component field coupled at first order,
$(\boldsymbol\sigma\cdot\boldsymbol\pi)^2 = \boldsymbol\pi^2 - e\hbar\,\boldsymbol\sigma\cdot\mathbf B$
(checked in the Landau representation), so $H = (\boldsymbol\sigma\cdot\boldsymbol\pi)^2/2m$
contains $-\mu_B\boldsymbol\sigma\cdot\mathbf B$: $\mu_B$ and $g = 2$ without a loop, and without
relativity \cite{levyleblond}. Two costs: the medium's doublet must transform as a spinor under
rotations, and the hadron anchor $2\pi\lb$ of Section~\ref{sec:nucleon} is no longer ``the
electron's circuit'' (that circuit is now $\pi\lb$) but a posed length; the hadronic numbers stand
only with it posed.
\item \textbf{Where the spinor sits (R105).} The three directions of the cubic weave cannot carry
it: a real reciprocal network ($T^2 = +1$) has no $\sigma_y$, and the six half-lines of an $O_h$
node decompose as $A_1 + E + T_1$, spin 0 and 1 only, with the doublet $E$ bosonic ($2\pi \to +1$).
The complex structure comes from one-way circulation ($J^2 = -1$, reversed by $T$), and the Pauli
algebra from the orientation of a circulating unit: with the SU(2) lift of R95,
$[\sigma_x/2, \sigma_y/2] = i\sigma_z/2$ is the composition law of orientation rotations. The
universal electron field is therefore the field of the medium's oriented one-way excitations (the
antiperiodic sector), not a mode of the weave; averaging the one-dimensional Weyl pair over unit
orientations gives $\langle(\boldsymbol\sigma\cdot\hat{\mathbf n})(\hat{\mathbf n}\cdot\mathbf p)\rangle
= \tfrac13\,\boldsymbol\sigma\cdot\mathbf p$: the Weyl form, at $c/3$.
\item \textbf{The node rule (R106--R107).} The base's node is the Johns symmetrical condensed node
\cite{johns} (\texttt{v2.9/scripts/johns/S\_johns.npy}), the unique isotropic node of its ansatz;
it is reproduced: two acoustic branches at $v = 1/2$ in link units, isotropic to $2\times10^{-7}$,
the base's $c_0$. A spinor label that follows the direction by a change of basis is inert (the SCN
bands doubled to $10^{-14}$, holonomy $+1$ on every lattice loop). The geometric transport (SU(2)
lift of the shortest rotation at each turn) gives $-1$ around a square loop but a non-unitary node
($\|S^\dagger S - I\| = 1.0$). The complete family of unitary equivariant nodes for a
helicity-locked spin-$\tfrac12$ excitation is enumerated exactly: on the six helicity states the
binary octahedral group acts as $j = \tfrac12 \oplus \tfrac32$ and the node is a phase per
component; with the SCN's two polarizations it acts as $2G + E_{1/2} + E'_{1/2}$ and, at first
order in $q$, the linear branches depend only on the eigenvalue-one subspace $E_1$ of the node,
which is a sum of irreducible components (any point of $\mathbb{CP}^1$ inside a block of
multiplicity two). Calibration: the enumeration for the photon ($2T_1 + 2T_2$) reproduces the SCN,
four branches at $v = 1/2$, and $1/2$ is the maximum of the photon family. Result: every isotropic
spinor node, with or without polarization, propagates a Weyl cone at $v = 1/3$ exactly (the
reduced matrix element of $\hat{\mathbf n}$ on $j = \tfrac12$ over the cubic directions), against
$1/2$ for the photon of the same weave: $v_e/c_0 = 2/3$, where the electron's maximal velocity
equals that of light to better than $10^{-11}$ \cite{hohensee}.
\end{enumerate}

The cubic weave of the base cannot carry both the spinor and the speed of light: a spinor at $c_0$
is inert (no $-1$), a spinor with the $-1$ goes at $2c_0/3$. The only other structure the base has,
the bundles of $n \ge 3$ strands of the earlier phase, was tested as the carrier (R108): the
differential doublet of an $n$-strand bundle carries a Berry charge $a(n)$ under rotation of its
frame ($0.186$, $0.329$, $0.432$, $0.507$, $0.564$, $0.608$ for $n = 3$ to $8$ at the matched
radius, and continuously varying with the conductor radius), so its transport along a path of
directions is $e^{\pm ia\Omega}$; a spinor needs $a = \tfrac12$ exactly. No bundle has it by
structure (the six-strand bundle is $1.4\,\%$ away at the matched radius and reaches $\tfrac12$
only at a tuned radius), a doublet with non-half-integer $a$ is not single-valued under $2\pi$
and cannot be transported by any equivariant node, and one with $a = \tfrac12$ would fall back to
$v = 1/3$. With the frame locked to the medium the closed three-strand bundle's holonomy is
exactly $120^\circ$, the generation label, never $\pi$. The field reading survives as a
consistent reading of the electron; the base has no carrier for it.

\status{Conditionally derived for the field identities of R104 (on the spinor nature of the
unit); Derived for the group-theoretic velocities $1/3$ and $1/2$ and for the Berry charges
$a(n)$; Closed for the electron field as an excitation of the cubic weave or of an $n \ge 3$
bundle}
...
that a loop magneton needs $R \ge \lb$; that the telegrapher pair is a Weyl pair and the half
quantum the antiperiodic sector; that the cubic weave's directions carry spin 0 and 1 only; the
velocities $1/3$ (spinor) and $1/2$ (photon) of the cubic weave. \\
\addlinespace
...
\addlinespace
Closed & the extended electron (charge and magneton loops); the electron field as an excitation
of the cubic weave ($v_e/c_0 = 2/3$) or of an $n \ge 3$ bundle (Berry charge $\ne \tfrac12$); a weak sector inside the base; dark matter from the medium;
the base as a local theory of Bell correlations; winding as confinement; the mass-as-inductance
law. \\
\bottomrule
\end{tabular}
...
\begin{enumerate}[nosep]
\item \textbf{The electron's carrier.} The field reading (R104) gives $S$, $\mu_B$, $g = 2$ and
$G_E = 1$ from a first-order two-component field, but the base's cubic weave transports a spinor
at $2c_0/3$ (R106--R107), and the $n \ge 3$ bundles carry a continuous Berry charge, not a
spinor (R108). What is needed is a medium whose node gives the spinor and the photon the same
limiting velocity; the base as written has no carrier. Until then the base derives none of $S$,
$\mu_B$, $g$.
...
\item \textbf{The exponent $2\pi$ and the count 9}, structural inputs; and, under the field
reading, the hadron anchor $2\pi\lb$, now a posed length rather than the electron's circuit.
...
Two parts of this sector have already been falsified by existing data: an electron whose charge
or magnetic moment is carried by a loop of any radius above $10^{-3}$~fm
(Section~\ref{sec:electron}), and an electron field carried by the base's cubic weave, whose
limiting velocity would be $2/3$ of light's (Section~\ref{sec:fieldquantum}). The paper reports
these as its firmest results.
...
in the session record and is to be filled in by the author) wrote the 104 scripts, ran them,
...
\bibitem{levyleblond} J.-M. L\'evy-Leblond, \textit{Nonrelativistic particles and wave equations}, Commun. Math. Phys. \textbf{6}, 286 (1967).
\bibitem{johns} P. B. Johns, \textit{A symmetrical condensed node for the TLM method}, IEEE Trans. Microwave Theory Tech. \textbf{35}, 370 (1987).
\bibitem{hohensee} M. A. Hohensee, R. Lehnert, D. F. Phillips and R. L. Walsworth, \textit{Particle-accelerator constraints on isotropic modifications of the speed of light}, Phys. Rev. Lett. \textbf{102}, 170402 (2009).
\end{thebibliography}
