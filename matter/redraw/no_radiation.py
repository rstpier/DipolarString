...
def main() -> int:
    ...
    check("(1) A point charge at the pole circling at v -> c_0 radiates without bound (Larmor x gamma^4): lifetimes far below one turn",
          all(t < PERIOD for _, _, t in rows[1:]), "; ".join(f"v = {b} c: P = {p:.1e} W, tau = {t:.1e} s" for b, p, t in rows) + f" (one turn = {PERIOD:.1e} s)")
